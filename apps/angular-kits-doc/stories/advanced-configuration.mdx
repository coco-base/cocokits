...
<MdxDocWithTheme fn={theme => [
  header(3, 'Step 1: Install the Components'),
  `Install the specific components and the ${theme.name} theme:`,
  <MdxDocTabCode codeTabs={{
    'npm': `npm i @cocokits/angular-core @cocokits/angular-button @cocokits/angular-radio @cocokits/theme-${theme.id}`,
    'yarn': `yarn add @cocokits/angular-core @cocokits/angular-button @cocokits/angular-radio @cocokits/theme-${theme.id}`,
    'pnpm': `pnpm add @cocokits/angular-core @cocokits/angular-button @cocokits/angular-radio @cocokits/theme-${theme.id}`
  }}/>,
  header(3, 'Step 2: Provide Theme Configuration'),
  `Provide the ${theme.name} theme configuration in the root of your application.`,
  getInstallPackagesStep2Provide(theme),
  header(3, `Step 3: Import Tokens and Component Styles`),
  `Instead of importing all styles, you can import only the necessary tokens and styles for the components you're using.`,
  header(4, 'Import Tokens'),
  'In your `angular.json` or `project.json`, add the tokens CSS file:',
  getInstallPackagesStep3AngularJson(theme),
  '**Note:** We are only importing the tokens here, not the global styles.',
  header(4, 'Import Component Styles'),
  'In your `styles.scss`, add the custom styles for the components you installed:',
  getInstallPackagesStep3ComponentStyle(theme),
...
<MdxDocWithTheme fn={getOverrideComponentConfig}/>
...
## Merge Themes
You can also merge multi themes and create a new theme. For example, you want to have the `Cocokits` theme but `button`, `radio` be from `FrameX`

### Step 1: Install the Themes

<MdxDocTabCode codeTabs={{
  'npm': `npm i @cocokits/angular-components @cocokits/theme-frame-x @cocokits/theme-default`,
  'yarn': `yarn add @cocokits/angular-components @cocokits/theme-frame-x @cocokits/theme-default`,
  'pnpm': `pnpm add @cocokits/angular-components @cocokits/theme-frame-x @cocokits/theme-default`
}}/>

### Step 2: Merge the Theme Configurations

<MdxDocWithTheme fn={getMergeThemesStep2Provide}/>
...
**styles.scss**
<MdxDocWithTheme fn={getMergeThemesStep3Scss}/>
...
#### Step 1: Import Modes
**`styles.scss`**
<MdxDocWithTheme fn={getUseOnlyOneModeOption1Step1Code}/>


#### Step 2: Add Default Selectors to Your HTML Tag
In your index.html, add the default selectors to the `<html>` tag:

<MdxDocWithTheme fn={getUseOnlyOneModeOption1Step2ClassSelector}/>
Alternatively, use the `data-cck-theme` attribute:
<MdxDocWithTheme fn={getUseOnlyOneModeOption1Step2AttrSelector}/>
...
<MdxDocWithTheme fn={getUseOnlyOneModeOption1code}/>
...
<MdxDocWithTheme fn={getChangeTokenSelectorsItems}/>

#### Step 1: Define Custom Selectors
**styles.scss**
<MdxDocWithTheme fn={getChangeTokenSelectorsStep1Code}/>

#### Step 2: Apply Custom Selectors
**index.html**
<MdxDocWithTheme fn={getChangeTokenSelectorsStep2Code}/>
