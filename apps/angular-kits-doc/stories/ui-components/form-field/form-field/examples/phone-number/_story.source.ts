...
\`;`,
  ...
  {
    language: 'html',
    filename: 'phone-number.component.html',
    code: `
<cck-form-field>
  <cck-label>Phone number</cck-label>
  <cck-prefix class="prefix">+{{ value() === 'TH' ? '66' : '43' }}</cck-prefix>
  <input cckInput [mask]="value() === 'TH' ? '999-999-999' : '999 9999 999'">
  <cck-suffix>
    <cck-select
      [value]="value()"
      (selectionChange)="value.set($event[0])">

      <cck-select-preview>
        <cck-svg-icon [size]="'<%=previewFlagSize%>'" [icon]="value() === 'TH' ? THFlag : ATFlag"></cck-svg-icon>
      </cck-select-preview>

      <cck-option value="TH">
        <div class="option-wrapper">
          <cck-svg-icon [size]="'<%=optionFlagSize%>'" [icon]="THFlag"></cck-svg-icon>
          TH
        </div>
      </cck-option>
      <cck-option value="AT">
        <div class="option-wrapper">
          <cck-svg-icon [size]="'<%=optionFlagSize%>'" [icon]="ATFlag"></cck-svg-icon>
          AT
        </div>
      </cck-option>
    </cck-select>
  </cck-suffix>
  <cck-hint>Enter your local phone number</cck-hint>
</cck-form-field>`,
  },
  {
    language: 'scss',
    filename: 'phone-number.component.scss',
    code: `
.prefix {
...
.option-wrapper {
  ...
  {
    language: 'angular-ts',
    filename: 'phone-number.component.ts',
    code: `
import { Component, input, signal } from '@angular/core';

import { NgxMaskDirective, provideNgxMask } from 'ngx-mask';

import { FormFieldComponent, HintComponent,InputComponent, LabelComponent, OptionComponent, PrefixComponent, SelectComponent,SelectPreviewComponent, SuffixComponent,SvgIconComponent } from '@cocokits/angular-components';
...
@Component({
  ...
  imports: [FormFieldComponent, LabelComponent, PrefixComponent, InputComponent, SelectComponent, OptionComponent, HintComponent, NgxMaskDirective, SuffixComponent, SvgIconComponent, SelectPreviewComponent],
