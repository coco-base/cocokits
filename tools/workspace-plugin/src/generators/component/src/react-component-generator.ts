...
export function reactComponentGenerator(tree: Tree, options: ComponentGeneratorOptions) {
  // Generate component
  generateFiles(
    tree,
    posixPath.join(__dirname, '../files/react-component'),
  ...
  if (options.export) {
    ...
    const exportComponent = `export * from './${componentDirectoryFromSrc}/${options.fileName}';`;
    updateTextFile(
      tree,
  ...
  if (options.story) {
    ...
    generateFiles(tree, posixPath.join(__dirname, '../files/react-story'), storyDir, options);
  }
