 ...
 */
export function fontWeightToNumber(value: string | number): string {
  const valueAsInt = typeof value === 'number' ? value : parseInt(value);

  if (!isNaN(valueAsInt) && fontWeightsNumbers.includes(valueAsInt)) {
    return `${value}`;
  }

  const fromMatrix: false | number | undefined = typeof value === 'string' && fontWeightsNames[value.toLowerCase()];

  if (fromMatrix) {
    return `${fromMatrix}`;
  }

  Logger.warning(`Unsupported font-weight: ${JSON.stringify(value)}. Use default font-weight: 400`);
  return '400';
}
