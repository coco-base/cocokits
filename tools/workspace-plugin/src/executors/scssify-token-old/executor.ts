...
export default async function runExecutor(options: ScssifyTokenExecutorSchema) {
  ...
  console.log('parserResult', parserResult.designTokenCollections['color-theme.theme-a'][0]);
  const compilerResult = compileDesignToken(parserResult, options);
  console.log('compilerResult', compilerResult.transformedTokens['color-theme.theme-a'][0]);
