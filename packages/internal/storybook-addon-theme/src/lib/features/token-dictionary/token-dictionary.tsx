              ...
              <tbody>
                {
                  tokensMaps[selectedCollection].map(tokenOrGroupName => {
                    ...
                    return (
                      <tr key={tokenOrGroupName.id} className={selectedTokenId === tokenOrGroupName.id ? 'selected' : ''} onClick={() => onTokenClick(tokenOrGroupName.id)}>
                          ...
                          modes.map(mode => {
                            ...
                            if(aliasTokenId) {
                              ...
                              return (
                                <td key={aliasToken.id}>
                                  <TokenDictionaryValue tokenId={aliasToken.id} type={aliasToken.type} text={aliasToken.namePath.join(' / ')} compact={true}/>
                                </td>
                              );
                            }
                            ...
                            return (
                              <td key={tokenOrGroupName.id}>
                                <TokenDictionaryValue tokenId={tokenOrGroupName.id} type={tokenOrGroupName.type} text={tokenOrGroupName.modes[mode].value} compact={true}/>
                              </td>
                            );
                          })
                        }
